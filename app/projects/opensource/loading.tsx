// // app/projects/opensource/loading.tsx
// export default function Loading() {
//   return (
//     <main className="container mx-auto px-4 py-12">
//       <section className="flex flex-col items-center justify-center py-24">
//         <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-green-500 border-solid mb-6"></div>
//         <h2 className="text-xl font-semibold text-gray-700">Loading open source projects…</h2>
//         <p className="text-gray-500 mt-2">Fetching your portfolio contributions.</p>
//       </section>
//     </main>
//   );
// }

// app/projects/opensource/loading.tsx
export default function Loading() {
  return (
    <main className="container mx-auto px-4 py-12">
      <section className="text-center py-12">
        <h1 className="text-4xl font-bold mb-4">Open Source Projects</h1>
        <ul className="space-y-6">
          {[...Array(3)].map((_, i) => (
            <li
              key={i}
              className="border p-4 rounded-lg shadow-sm animate-pulse"
            >
              <div className="h-6 bg-gray-300 rounded w-1/3 mx-auto mb-4"></div>
              <div className="h-4 bg-gray-200 rounded w-2/3 mx-auto mb-2"></div>
              <div className="h-4 bg-gray-200 rounded w-1/2 mx-auto"></div>
            </li>
          ))}
        </ul>
      </section>
    </main>
  );
}
